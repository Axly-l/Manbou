...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    public void onUpdateWalkingPlayer() {
        ...
        final ActionEvent actionEvent = new ActionEvent(this.isSprinting(), this.isSneaking());
        ...
        if (this.isCurrentViewEntity()) {
            ...
            final double xDiff = motionEvent.getPosX() - this.lastReportedPosX;
            final double yDiff = motionEvent.getPosY() - this.lastReportedPosY;
            final double zDiff = motionEvent.getPosZ() - this.lastReportedPosZ;
    ...
    {
        final StrafeEvent event = new StrafeEvent(this.rotationYaw, strafe, forward, friction);
        if(EventManager.INSTANCE.call(event)) return;

        final float rotationYaw = event.getYaw();
    ...
    public void onLivingUpdate()
    {
        ...
        boolean canSprint = RotationManager.INSTANCE.canSprint();
        boolean sprinting = (GameSettings.isKeyDown(mc.gameSettings.keyBindSprint) || AutoSprint.INSTANCE.getState());
        if (this.onGround && !sneaking && canSprint && !this.isSprinting() && flag3 && !this.isUsingItem() && !this.isPotionActive(Potion.blindness) && sprinting)
        {
        ...
        if (!this.isSprinting() && canSprint && flag3 && !this.isUsingItem() && !this.isPotionActive(Potion.blindness) && sprinting)
        {
            this.setSprinting(true);
        }

        if (this.isSprinting() && (!canSprint || this.isCollidedHorizontally || !flag3))
        {
            this.setSprinting(false);
        }

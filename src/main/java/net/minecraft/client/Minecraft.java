...
import net.ax.manbou.module.impl.world.Boost;
...
public class Minecraft implements IThreadListener, IPlayerUsage
{
    ...
    {
        GuiScreen oldScreen = this.currentScreen;

        if (this.currentScreen != null)
        {
            this.currentScreen.onGuiClose();
        }
    ...
    private void runGameLoop() throws IOException
    {
        ...
        {
            if(this.timer.hagehage > 0) {
                --this.timer.hagehage;
                continue;
            }

            EventManager.INSTANCE.call(new TickEvent(EventPhase.PRE));
            this.runTick();
            EventManager.INSTANCE.call(new TickEvent(EventPhase.POST));
            if(Boost.INSTANCE.getBoosting()){
                for(int k = 0; k < Boost.INSTANCE.getTick().getValue(); ++k) {
                    EventManager.INSTANCE.call(new TickEvent(EventPhase.PRE));
                    this.runTick();
                    EventManager.INSTANCE.call(new TickEvent(EventPhase.POST));
                    this.timer.hagehage += 1;
                }
                Boost.INSTANCE.toggle();
            }
        }
    ...
    private void clickMouse()
    {
        if (this.leftClickCounter <= 0)
        {
            ...
            else
            {
                ...
                {
                    case ENTITY:
                        if(EventManager.INSTANCE.call(new AttackEvent(this.thePlayer, this.objectMouseOver.entityHit))) break;
                        this.thePlayer.swingItem();
    ...
    public void runTick() throws IOException
    {
        ...
        if (this.currentScreen == null || this.currentScreen.allowUserInput)
        {
            ...
            EventManager.INSTANCE.call(new SignalEvent("clicking"));
    ...
    public void dispatchKeypresses()
    {
        ...
        if(Keyboard.getEventKey() != 0) {
            EventManager.INSTANCE.call(new KeyboardEvent(i, Keyboard.isKeyDown(Keyboard.getEventKey())));
        }
    }

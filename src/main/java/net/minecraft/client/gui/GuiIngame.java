...
public class GuiIngame extends Gui
{
    ...
    private void renderScoreboard(ScoreObjective objective, ScaledResolution scaledRes)
    {
        ...
        for (Score score1 : collection)
        {
            ...
            if (j == collection.size())
            {
                ...
                this.getFontRenderer().drawString(s3, l1 + i / 2 - this.getFontRenderer().getStringWidth(s3) / 2, k - this.getFontRenderer().FONT_HEIGHT, 0xffffffff);
            }
        }
    }
    ...
    {
        if(FullBright.INSTANCE.getState() && FullBright.INSTANCE.getNoPumpkinBlur().getValue()) return;
    ...
    {
        if(FullBright.INSTANCE.getState()) return;

...
import net.ax.manbou.event.Render3DEvent;
import net.ax.manbou.module.impl.combat.Reach;
import net.ax.manbou.module.impl.visual.FullBright;
import net.ax.manbou.utils.RotationManager;
import net.ax.manbou.utils.RotationsKt;
...
public class EntityRenderer implements IResourceManagerReloadListener
{
    ...
    public void getMouseOver(float partialTicks)
    {
        ...
        if (entity != null)
        {
            if (this.mc.theWorld != null)
            {
                ...
                if (this.pointedEntity != null && flag && vec3.distanceTo(vec33) > (Reach.INSTANCE.getState()? Reach.INSTANCE.getReach() : 3.0D))
                {
                    this.pointedEntity = null;
    ...
    {
        RotationManager r = RotationManager.INSTANCE;
        Entity entity = this.mc.getRenderViewEntity();
        ...
        if (entity instanceof EntityLivingBase && ((EntityLivingBase)entity).isPlayerSleeping())
        {
            ...
            if (!this.mc.gameSettings.debugCamEnable)
            {
                ...
                GlStateManager.rotate(r.getPrevCameraYaw() + (r.getCameraYaw() - r.getPrevCameraYaw()) * partialTicks + 180.0F, 0.0F, -1.0F, 0.0F);
                GlStateManager.rotate(r.getPrevCameraPitch() + (r.getCameraPitch() - r.getPrevCameraPitch()) * partialTicks, -1.0F, 0.0F, 0.0F);
            }
        }
        else if (this.mc.gameSettings.thirdPersonView > 0)
        {
            ...
            {
                float f1 = r.getCameraYaw();
                float f2 = r.getCameraPitch();
                ...
                GlStateManager.rotate(r.getCameraPitch() - f2, 1.0F, 0.0F, 0.0F);
                GlStateManager.rotate(r.getCameraYaw() - f1, 0.0F, 1.0F, 0.0F);
                GlStateManager.translate(0.0F, 0.0F, (float)(-d3));
                GlStateManager.rotate(f1 - r.getCameraYaw(), 0.0F, 1.0F, 0.0F);
                GlStateManager.rotate(f2 - r.getCameraPitch(), 1.0F, 0.0F, 0.0F);
            }
        }
        else
        {
        ...
        if (!this.mc.gameSettings.debugCamEnable)
        {
            GlStateManager.rotate(r.getInterpolatedCameraPitch(partialTicks), 1.0F, 0.0F, 0.0F);
            ...
            {
                GlStateManager.rotate(r.getInterpolatedCameraYaw(partialTicks) + 180.0F, 0.0F, 1.0F, 0.0F);
            }
        }
        ...
        if (this.lightmapUpdateNeeded)
        {
            ...
            if (world != null)
            {
                ...
                for (int i = 0; i < 256; ++i)
                {
                    ...
                    this.lightmapColors[i] = FullBright.INSTANCE.getState() ? 0xFFFFFFFF : j << 24 | k << 16 | l << 8 | i1;
                }
    ...
    public void updateCameraAndRender(float partialTicks, long nanoTime)
    {
        ...
        if (this.mc.inGameHasFocus && flag)
        {
            ...
            float gcd = RotationsKt.getGcd();
            float deltaYaw = (float)this.mc.mouseHelper.deltaX * gcd;
            float deltaPitch = (float)this.mc.mouseHelper.deltaY * gcd;

            if (this.mc.gameSettings.smoothCamera)
            {
                this.smoothCamYaw += deltaYaw;
                this.smoothCamPitch += deltaPitch;
                float deltaPartialTicks = partialTicks - this.smoothCamPartialTicks;
                this.smoothCamPartialTicks = partialTicks;
                deltaYaw = this.smoothCamFilterX * deltaPartialTicks;
                deltaPitch = this.smoothCamFilterY * deltaPartialTicks;
            }
            else
            {
                this.smoothCamYaw = 0.0F;
            ...
            RotationManager.INSTANCE.setAngles(deltaYaw, deltaPitch);
            EventManager.INSTANCE.call(new CameraEvent(partialTicks));
        }

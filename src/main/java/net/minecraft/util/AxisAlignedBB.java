...
public class AxisAlignedBB
{
    ...
    public AxisAlignedBB expandMultiplier(double x, double y, double z) {
        double xDiff = this.maxX - this.minX;
        double yDiff = this.maxY - this.minY;
        double zDiff = this.maxZ - this.minZ;

        double d0 = this.minX - xDiff * (x - 1.0) / 2.0;
        double d1 = this.minY - yDiff * (y - 1.0) / 2.0;
        double d2 = this.minZ - zDiff * (z - 1.0) / 2.0;
        double d3 = this.maxX + xDiff * (x - 1.0) / 2.0;
        double d4 = this.maxY + yDiff * (y - 1.0) / 2.0;
        double d5 = this.maxZ + zDiff * (z - 1.0) / 2.0;
    ...
    public Vec3 getClosestPoint(Vec3 targetPos) {
        double x = Math.max(this.minX, Math.min(targetPos.xCoord, this.maxX));
        double y = Math.max(this.minY, Math.min(targetPos.yCoord, this.maxY));
        double z = Math.max(this.minZ, Math.min(targetPos.zCoord, this.maxZ));
